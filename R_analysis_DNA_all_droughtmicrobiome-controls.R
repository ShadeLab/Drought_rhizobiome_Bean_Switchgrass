...
tax = as.matrix(tax)
metadata = read.csv("sample-metadata-negcontrol.csv", sep=",", row.names=1)
...
##12607 reads total in negative controls after removing chloroplast and mitochondria
...
phyloseq_rarefy<- rarefy_even_depth(phyloseq_merged_clean, sample.size = 125, rngseed = TRUE, trimOTUs=FALSE)

#19 samples removed out of 48 because they contained fewer reads than `sample.size`.
#Up to first five removed samples are: 
#DNA_103_negDNA_104_negDNA_132_negDNA_144_negDNA_162_neg
...
phyloseq_genus <- phyloseq_rarefy %>%
...
View(phyloseq_class)
# Plot 
plot<- ggplot(phyloseq_class, aes(x= Sample, y = Abundance, fill =Class)) + 
  ...
  #
  guides(fill = guide_legend(reverse = TRUE, keywidth = 1, keyheight = 1, ncol=2)) +
  ylab("Relative Abundance (class > 1%) \n") +
  ggtitle("Class Composition")
plot
ggsave(filename = "barplot_class_negcontrols_bean_SW.tiff", plot = plot,
        width = 50,
...
Df SumsOfSqs MeanSqs F.Model      R2 Pr(>F)  
drought     2    0.3072 0.15361  1.4031 0.02703  0.018 *
...
pairwise.adonis2(phyloseq_bray ~ treatment, data = sampledf)
